...
class AccountAnalyticPlanVersion(models.Model):
    ...
    default_committed = fields.Boolean(
        "Default version for committed costs", default=False
    )
    ...
    def _check_default_committed(self):
        for _rec in self:
            ...
            if len(default_committed) > 1:
                raise UserError(_("Only one default commitments version can exist."))
            return
    ...
    def _check_default_plan(self):
        for _rec in self:
            ...
            if len(default_plan) > 1:
                raise UserError(_("Only one default plan version can exist."))
            return
