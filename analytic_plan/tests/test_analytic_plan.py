...
class TestAnalyticPlan(TransactionCase):
    def setUp(self):
        ...
        self.analytic_plan_journal_obj = self.env["account.analytic.plan.journal"]
        ...
        self.general_account_id = self.account_obj.search(
            [("user_type_id", "=", self.revenue.id)], limit=1
        )
