...
class AnalyticPlanCopyVersion(models.TransientModel):
    ...
    source_version_id = fields.Many2one(
        "account.analytic.plan.version", "Source Planning Version", required=True
    )
    dest_version_id = fields.Many2one(
        "account.analytic.plan.version", "Destination Planning Version", required=True
    )
    ...
    def analytic_plan_copy_version_open_window(self):
        ...
        active_model = self._context and self._context.get("active_model", False)
        ...
        if dest_version.default_plan:
            raise UserError(
                _("It is prohibited to copy " "to the default planning version.")
            )
        ...
        line_plans = line_plan_obj.search(
            [("account_id", "in", account_ids), ("version_id", "=", source_version.id)]
        )
