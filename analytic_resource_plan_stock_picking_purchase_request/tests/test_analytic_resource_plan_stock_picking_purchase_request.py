...
class TestAnalyticResourcePlanStockPickingPurchaseRequest(
    ...
    @classmethod
    def setUpClass(cls):
        super(TestAnalyticResourcePlanStockPickingPurchaseRequest, cls).setUpClass()
        ...
        cls.analytic_plan_journal_obj = cls.env["account.analytic.plan.journal"]
    ...
    def test_analytic_resource_plan_stock_picking(self):
        ...
        upd_qty = self.env["stock.change.product.qty"].create(
            {
                ...
                "product_tmpl_id": self.resource_plan_line.product_id.product_tmpl_id.id,
                "new_quantity": 1.0,
        ...
        purchase_request_line = self.resource_plan_line.purchase_request_lines[0]
        purchase_request = purchase_request_line.request_id
        purchase_request.button_to_approve()
        purchase_request.button_approved()
        self.assertEqual(
            self.resource_plan_line.request_state, "approved", "should approved request"
        )
        self.assertEqual(
