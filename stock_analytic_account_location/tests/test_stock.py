...
class TestStock(TransactionCase):
    def setUp(self):
        ...
        self.yourcompany_aa = self.env.ref("analytic.analytic_commercial_marketing")
        ...
        self.location3 = self.create_location(None, self.shelf_loc, "internal")
        self.location4 = self.create_location(None, None, "customer")
        self.location5 = self.create_location(self.AA2, False, "customer")
        ...
        self.cost_of_revenue = self.account_type_model.search(
            [("name", "=", "Cost of Revenue")], limit=1
        )
        self.current_liabilities_type = self.account_type_model.search(
        ...
        self.move1 = self.create_move("out_aa", self.location1, self.location4)
        self.move2 = self.create_move("out", self.location3, self.location4)
        self.move3 = self.create_move("in_aa", self.location4, self.location1)
        self.move4 = self.create_move("in", self.location4, self.location3)
    ...
    def create_analytic(self, name):
        vals = {"name": "name", "parent_id": self.yourcompany_aa.id}
        analytic = self.analytic_model.create(vals)
        return analytic

    def create_location(self, analytic, parent, usage):
        vals = {
            "name": analytic.name if analytic else "no analytic",
    ...
    def test_move_anaytic(self):
        ...
        self.assertEqual(
            self.move1.analytic_account_id.id,
            self.AA1.id,
            "Analytic account 1 should exist",
        )
        self.assertEqual(
            self.move2.analytic_account_id.id,
            False,
            "No analytic account should be in this move",
        )
        self.assertEqual(
            self.move3.analytic_account_id.id,
            self.AA1.id,
            "Analytic account 1 should exist",
        )
        self.assertEqual(
            self.move4.analytic_account_id.id,
            False,
            "No analytic account should be in this move",
        )
        self.assertEqual(
            self.move5.analytic_account_id.id,
            self.AA1.id,
            "Analytic account 1 should exist",
        )
        self.assertEqual(
            self.move6.analytic_account_id.id,
            self.AA1.id,
            "Analytic account 1 should exist",
        )
        with self.assertRaises(ValidationError):
            self.move7 = self.create_move("bad_aa", self.location2, self.location5)
