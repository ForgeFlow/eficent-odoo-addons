...
class AnalyticResourcePlanLine(models.Model):
    ...
    def _prepare_consume_move(self, line, product_qty):
        if line.product_id.type not in ("product", "consu"):
            raise ValidationError(
                _(
                    """The product must be stockable or
                consumable."""
                )
            )
        if product_qty <= 0:
            raise ValidationError(
                _(
                    """The quantity to consume must be greater
                or equal to 0."""
                )
            )
        ...
        if line.state != "confirm":
            raise ValidationError(
                _(
                    """The resource plan line must be
                confirmed."""
                )
            )
        ...
        if not line.account_id.location_id:
            raise ValidationError(
                _(
                    """The analytic account must contain a
                default Location."""
                )
            )
    ...
    def _prepare_produce_move(self, line, product_qty):
        ...
        if product_qty <= 0:
            raise ValidationError(
                _(
                    """The quantity to produce must be greater
                or equal to 0."""
                )
            )
        if line.state != "confirm":
