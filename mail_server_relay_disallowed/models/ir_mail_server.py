...
class IrMail_Server(osv.Model):
    ...
    def send_email(self, cr, uid, message, mail_server_id=None,
        ...
        if not smtp_from:
            smtp_from = self._get_default_bounce_address(cr, uid,
                                                         context=context)
        if not smtp_from:
            smtp_from = message['From']
        assert smtp_from, "The Return-Path or From header is required for " \
                          "any outbound email"

        # The email's "Envelope From" (Return-Path), and all recipient
        # addresses must only contain ASCII characters.
        from_rfc2822 = extract_rfc2822_addresses(smtp_from)
        assert from_rfc2822, ("Malformed 'Return-Path' or 'From' "
                              "address: %r - "
                              "It should contain one valid plain "
                              "ASCII email") % smtp_from
        ...
        smtp_to_list = filter(None, tools.flatten(map(
            extract_rfc2822_addresses,[email_to, email_cc, email_bcc])))
        ...
        if mail_server:
            ...
            if smtp_encryption is None and tools.config.get('smtp_ssl'):
                smtp_encryption = 'starttls' # STARTTLS is the new meaning
                # of the smtp_ssl flag as of v7.0
        ...
        try:
            ...
            try:
                ...
                (oldname,oldemail) = parseaddr(message['From']) #exact name
                # and address
                newfrom = formataddr((oldname,smtp_user)) #use original
                # name with new address
                message.replace_header('From', newfrom) #need to use
                # replace_header instead '=' to prevent double field
