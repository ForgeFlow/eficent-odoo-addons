...
class ProgressMeasurementsEntry(models.TransientModel):
    ...
    communication_date = fields.Date(required=True, default=fields.Date.context_today)
    ...
    def progress_measurements_open_window(self):
        ...
        progress_measurement_type_id = (
            data.get("progress_measurement_type_id", False)
            and data["progress_measurement_type_id"][0]
            or False
        )
        self._cr.execute(
            "SELECT DISTINCT ON (a.project_id) project_id, id, "
