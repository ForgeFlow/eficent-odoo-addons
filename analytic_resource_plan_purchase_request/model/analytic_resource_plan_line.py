...
class AnalyticResourcePlanLine(models.Model):
    ...
    def _get_request_state(self):
        for line in self:
            ...
            if line.purchase_request_lines:
                if any(
                    [
                        pr_line.request_id.state == "approved"
                    ...
                    [
                        pr_line.request_id.state == "rejected"
                        for pr_line in line.purchase_request_lines
                    ]
                ):
                    ...
                    [
                        pr_line.request_id.state in ("to_approve", "cancel")
                        for pr_line in line.purchase_request_lines
                    ]
                ):
                    ...
                    [
                        pr_line.request_id.state in ("draft", "cancel")
                        for pr_line in line.purchase_request_lines
                    ]
                ):
    ...
    label = fields.Char(
        "Label to PR", readonly=True, states={"draft": [("readonly", False)]}
    )
        ...
        for line in self:
            if line.purchase_request_lines.filtered(
                ...
                raise ValidationError(
                    _(
                        "You cannot delete plan lines that refers to "
