...
class AnalyticResourcePlanLineMakePurchaseRequest(models.TransientModel):
    ...
    def _prepare_purchase_request(
        self, make_purchase_request, picking_type_id, company_id
    ):
        data = {
    ...
    def make_purchase_request(self):
        ...
        for item in make_purchase_request.item_ids:
            ...
            line.write(values)
            project_manager_id = (
                line.account_id.user_id
                and line.account_id.user_id.partner_id.id
                or False
            )
            res.append(request_line.id)
            if project_manager_id:
                message_follower_ids = [
                    x.partner_id.id for x in request.message_follower_ids
                ]
