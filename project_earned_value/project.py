...
class project(osv.osv):
    ...
    def _earned_value(self, cr, uid, ids, names, arg, context=None):
        ...
        for project_id in wbs_projects_data.keys():
            ...
            if def_meas_type_ids:
                ...
                res[project_id].update(self._get_evm_ratios(ac, pv, ev,
                                                            total_bac))
    ...
    _columns = {
        ...
        'ev': fields.function(_earned_value, method=True, multi=_earned_value,
                              ...
                              digits_compute=dp.get_precision('Account'),
                              help="""Earned Value (PV) or Budgeted Cost
                              of Work Performed is the amount of work that has
                              been completed to date, expressed as the planned
                              value for that work."""),
        'ac': fields.function(_earned_value, method=True, multi=_earned_value,
                              string='AC', type='float',
                              digits_compute=dp.get_precision('Account'),
                              help="""Actual Cost (AC) or Actual Cost of
                              Work Performed is an indication of the level
                              of resources that have been expended to achieve
                              the actual work performed to date."""),
        'cv': fields.function(_earned_value, method=True, multi=_earned_value,
                              string='CV', type='float',
                              digits_compute=dp.get_precision('Account'),
                              help="""Cost Variance (CV) shows whether a
                              project is under or over budget.
                              It is determined as EV - AC.
                              A negative value indicates that the project is
                              over budget."""),
        'cvp': fields.function(_earned_value, method=True,
                               multi=_earned_value,
                               digits_compute=dp.get_precision('Account'),
                               help="""Cost Variance % (CVP) shows whether a
                               project is under or over budget.
                               It is determined as CV / EV.
                               A negative value indicates that the project
                               is over budget."""),
        'cpi': fields.function(_earned_value, method=True,
                               multi=_earned_value,
                               string='CPI', type='float',
                               digits_compute=dp.get_precision('Account'),
                               help="""Cost Performance Index (CPI) indicates
                               how efficiently the team is using its
                               resources. It is determined as EV / AC.
                               A value of 0.8 indicates that the project has
                               a cost efficiency that provides 0.8 worth of
                               work for every unit spent to date."""),
        'tcpi': fields.function(_earned_value, method=True,
                                multi=_earned_value,
                                string='TCPI', type='float',
                                digits_compute=dp.get_precision('Account'),
                                help="""To-Complete Cost Performance Index
                                (TCPI) helps the team determine the
                                efficiency that must be achieved on the
                                remaining work for a project to meet the
                                Budget at Completion (BAC). It is determined
                                as (BAC - EV) / (BAC - AC)"""),
        'sv': fields.function(_earned_value, method=True,
                              multi=_earned_value,
        ...
        'svp': fields.function(_earned_value, method=True,
                               ...
                               digits_compute=dp.get_precision('Account'),
                               help="""Schedule Variance % (SVP) determines
                               whether a project is ahead or behind schedule.
                               It is calculated as SV / PV.
                               A negative value indicates what percent of the
                               planned work has not been accomplished"""),
        'spi': fields.function(_earned_value, method=True,
                               multi=_earned_value,
                               string='SPI', type='float',
                               digits_compute=dp.get_precision('Account'),
                               help="""Schedule Performance Index (SPI)
                               indicates how efficiently the project team
                               is using its time. It is calculated as
                               EV / PV. For example, on a day, indicates
                               how many hours worth of the planned work
                               is being performed."""),
        'eac': fields.function(_earned_value, method=True,
                               multi=_earned_value,
                               string='EAC', type='float',
                               digits_compute=dp.get_precision('Account'),
                               help="""Estimate at Completion (EAC) provides
                               an estimate of the final cost of the project
                               if current performance trends continue.
                               It is calculated as BAC / CPI."""),
        'etc': fields.function(_earned_value, method=True,
                               multi=_earned_value,
                               string='ETC', type='float',
                               digits_compute=dp.get_precision('Account'),
                               help="""Estimate to Complete (ETC) provides
                               an estimate of what will the remaining
                               work cost. It is calculated as
                               (BAC - EV) / CPI."""),
        'vac': fields.function(_earned_value, method=True,
                               multi=_earned_value,
                               string='VAC', type='float',
                               digits_compute=dp.get_precision('Account'),
                               help="""Variance at Completion (VAC) shows
                               the team whether the project will finish
                               under or over budget.
                               It is calculated as BAC - EAC."""),
        'vacp': fields.function(_earned_value, method=True,
                                multi=_earned_value,
                                string='VACP', type='float',
                                digits_compute=dp.get_precision('Account'),
                                help="""Variance at Completion % (VACP)
                                shows the team whether the project will
                                finish under or over budget.
                                It is calculated as VAC / BAC."""),
        'bac': fields.function(_earned_value, method=True,
                               multi=_earned_value,
                               string='BAC', type='float',
                               digits_compute=dp.get_precision('Account'),
                               help="Budget at Completion (BAC)"),
        'pcc': fields.function(_earned_value, method=True,
                               multi=_earned_value,
                               string='PCC', type='float',
                               digits_compute=dp.get_precision('Account'),
                               help="Costs to date / Total costs"),

        'poc': fields.function(_earned_value, method=True,
                               multi=_earned_value,
    ...
    def update_project_evm(self, cr, uid, project_ids,
        ...
        progress_measurement_type_obj = \
            self.pool.get('progress.measurement.type')

        progress_measurement_type = progress_measurement_type_obj.browse(
        ...
        for project_id in wbs_projects_data.keys():
            ...
            project_evm_ids = project_evm_obj.search(
            ...
            for day_datetime in l_days:
                ...
                cr.execute('SELECT project_id, value '
                           ...
                           (tuple(wbs_project_ids), day_date,
                            progress_measurement_type_id))

                res_project_measurements = cr.fetchall()
                pm_at_date = {}
                for res_project_meas in res_project_measurements:
                    pm_at_date[res_project_meas[0]] = \
                        res_project_meas[1]
                ...
                for pd_project_id, \
                    ...
                    if pd_project_id \
                            in pm_at_date:
                        # If the project is completed on this date,
                        # then record earned value
                        # as a function of the progress.
                        progress_value = \
                            pm_at_date[pd_project_id]
                        if pd_project_id in projects_total_amount:
                            ev_projects[pd_project_id] = \
                                projects_total_amount[pd_project_id] * \
                                progress_value / \
                                progress_max_value
                ...
                records.extend(project_obj.create_evm_record(
                    cr, uid, project_id, day_date, ratios))
            return records

    def create_evm_record(self, cr, uid, project_id, eval_date, ratios,
                          context=None):
        records = []
        project_evm_obj = self.pool.get('project.evm')
