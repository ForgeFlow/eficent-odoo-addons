...
class AnalyticResourcePlanLine(models.Model):
    ...
    has_child = fields.Boolean(compute="_compute_has_child", string="Has child")
    ...
    price_unit = fields.Float(
        string="Cost Price", groups="project.group_project_manager"
    )
    ...
    def _prepare_analytic_lines(self):
        ...
        default_plan = plan_version_obj.search([("default_plan", "=", True)], limit=1)
    ...
    def unlink(self):
        for line in self:
            if line.analytic_line_plan_ids:
                raise ValidationError(
                    _(
                        "You cannot delete a record that refers to analytic plan"
                        " lines"
                    )
                )
