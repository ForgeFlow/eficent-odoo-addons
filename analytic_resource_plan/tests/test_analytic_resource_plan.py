...
class TestAnalyticResourcePlan(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.project = cls.env["project.project"].create(
            {"name": "Test project", "code": "X0001"}
        )
        cls.account_id = cls.project.analytic_account_id
        cls.plan_version = cls.env.ref("analytic_plan.analytic_plan_version_P02")
        cls.account_id.write({"active_analytic_planning_version": cls.plan_version.id})
        ...
        cls.product.categ_id.property_account_expense_categ_id = cls.general_account_id
        ...
        cls.plan_expenses = cls.env["account.analytic.plan.journal"].create(
            {"name": "expenses", "code": "EXP", "analytic_journal": cls.anal_journal.id}
        )
