...
class TestChanges(TransactionCase):
    def setUp(self):
        ...
        self.test_project_id = self.project_model.create({"name": "ChangeTestProject"})
        self.change_owner_id = self.env.ref("change_management.user_change_user")
        self.change_second_author_id = self.change_owner_id.copy()
        self.change_manager_id = self.env.ref("change_management.user_change_manager")
        self.change_partner_manager = self.env.ref(
        ...
        self.test_change_id = self.change_model.create(
            {
                ...
                "author_id": self.change_owner_id.id,
                "change_owner_id": self.change_manager_id.id,
            }
        )
    ...
    def test_change_owner_and_creator_added_to_followers_for_change(self):
        ...
        self.assertTrue(
            len(followers) == 1, msg="Expecting 3 followers - got:%s" % len(followers)
        )

    def test_saving_a_change_in_users_as_followers_works(self):
        self.test_change_id.write({"author_id": self.change_second_author_id.id})
        change = self.test_change_id
        followers = [follower.id for follower in change.message_follower_ids]
        self.assertTrue(
            len(followers) == 2, msg="Expecting 4 followers - got:%s" % len(followers)
        )

    def test_adding_a_task_on_a_change(self):
        change = self.test_change_id.read(["message_follower_ids"])
        followers = self.env["mail.followers"].browse(change[0]["message_follower_ids"])
                ...
                "change_response_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "date_start": False,
                            "company_id": 1,
                            "message_follower_ids": [(4, followers.id)],
                            "project_id": 1,
                            "partner_id": False,
                            "message_ids": False,
