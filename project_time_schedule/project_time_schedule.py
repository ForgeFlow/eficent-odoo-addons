...
class task(osv.osv):
    ...
    def onchange_stage_id(self, cr, uid, ids, stage_id, context=None):
        ...
        state = cr.fetchone()[0] or False
        if state in ('cancelled','done'):            
            return {'value': {'date_end': fields.datetime.now()}}
        elif state in ('open'):
            return {'value': {'date_start': fields.datetime.now()}}
        else:
            return {'value': {}}
    def get_network(self, cr, uid, ids, d_activities, *args):
        
        read_data = []
        read_data = self.read(cr, uid, ids[0], ['duration', 'successor_ids', 'predecessor_ids', 'date_earliest_start', 'date_latest_finish', 'date_start', 'date_end', 'state'])
        ...
        successor_ids = read_data['successor_ids']
        predecessor_ids = read_data['predecessor_ids']

        # If task has started we consider it a start restriction
        
        if read_data['state'] in ('open') and read_data['date_start']:
        ...
        for parent_id in predecessor_ids:                  
    ...
    def calculate_network(self, cr, uid, ids, context, *args):
        for task in self.browse(cr, uid, ids, context=context):
            ...
            for parent in task.predecessor_ids:
                self.calculate_network_task(cr, uid, [parent.id], context=context)
            for child in task.successor_ids:
        ...
        for task_id in d_activities.keys():
            if (not task_id == 'start') and (not task_id == 'stop'):                
                ...
                task_obj.write(cr, uid, task_id, {
                    'date_start': d_activities[task_id].date_early_start,
                    'date_end': d_activities[task_id].date_early_finish,
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        #if not context.get('calculate_network') and ids and (vals.get('duration') or vals.get('state') or vals.get('date_latest_finish') or vals.get('date_earliest_start') or vals.get('predecessor_ids') or vals.get('successor_ids')):
...
#            if position.date_early_start < position.date_earliest_start:
        ...
        if position.is_start:                        
            position.date_early_finish = network_activity.add_work_days(position.date_early_start, position.replan_duration)
