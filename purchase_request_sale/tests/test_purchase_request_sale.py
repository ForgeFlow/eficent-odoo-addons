...
class TestPurchaseRequestProcurement(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        product_values = {"name": "Odoo", "list_price": 5, "type": "product"}
        cls.product = cls.env["product.product"].create(product_values)
        cls.product_uom_unit = cls.env.ref("uom.product_uom_unit")
        cls.product.route_ids = cls.env.ref("stock.route_warehouse0_mto") | cls.env.ref(
            "purchase_stock.route_warehouse0_buy"
        )
        for rule in cls.env.ref("purchase_stock.route_warehouse0_buy").rule_ids:
            rule.propagate = True
    ...
    def test_propagate(self):
        ...
        sale.action_cancel()
        self.assertEqual(sale.purchase_request_ids[0].state, "rejected", "not rejected")
